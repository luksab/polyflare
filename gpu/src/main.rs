...
use gpu::lens_state::GlassElement;
use gpu::*;
use polynomial_optics::Lens;
use state::State;
...
fn main() {
    let num_terms = 100;
    let lens = gpu::lens_state::LensState::get_lens_els();
    let sensor_dist = 3.;
    let lens = Lens::new(gpu::lens_state::LensState::get_lens_arr(&lens), sensor_dist);
    let num_dots = 10_000;
    let degree = 10;
    ...
    for num_threads in [1, 4, 8, 16, 32, 48].iter() {
            ...
            .install(|| {
                ...
                vec![(1usize, 0usize)]
                    ...
                    .map(|(which_ghost, dir_xy)| {
                        ...
                        let polynom = polynomial_optics::Polynom4d::<_>::fit(&points, degree);
                        ...
                        println!("{}: {} took {}", num_threads, sparse_poly.error(&points_rand), now.elapsed().as_millis());
                    })
                    .collect::<Vec<_>>()
            });
    }
